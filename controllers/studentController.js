...
const getModules = async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT DISTINCT m.*, t.name AS teacher_name, g.name AS group_name, g.year AS group_year, g.id AS group_id
       FROM modules m
...
const getSubmissions = async (req, res) => {
  try {
    ...
    const [workshopSubs] = await pool.query(
      `SELECT ws.*, ws.reaction AS teacher_reaction, w.title AS workshop_title, m.title AS module_title, g.name AS group_name, g.year AS group_year
    ...
    const [sprintSubs] = await pool.query(
      `SELECT ss.*, ss.reaction AS teacher_reaction, sp.title AS sprint_title, m.title AS module_title, at.name AS team_name, g.name AS group_name, g.year AS group_year
